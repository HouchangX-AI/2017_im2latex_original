...
class Trainer:
    def __init__(self, model, train_loader, config):
        ...
        self.criterian = nn.NLLLoss(ignore_index=train_loader.vocab.pad_token)   #对数
        self.optimizer = optim.Adam(self.model.parameters(), lr=config['learning_rate'])    #优化模型
        self.scheduler = optim.lr_scheduler.StepLR(self.optimizer, step_size=config['learning_rate_decay_step'],
                                                    gamma=config['learning_rate_decay'])    #每次迭代的学习率为初始学习率的某个函数倍
        self.learning_rate_min = config['learning_rate_min']

        self.checkpoints_dir = config['checkpoints_dir']    #使用checpoints不会保留中间变量
    ...
    def train_one_epoch(self):
        ...
        while not epoch_ended:
            ...
            loss.backward()  # 反向传播
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.clip)   # 梯度裁剪，防止梯度消失
            self.optimizer.step()
